...
**IMPORTANT: Always test against the latest release version.** Check with `npm view <package> version` (or equivalent). Do NOT use the version from the original report. The goal is to determine whether the issue exists *now*.
